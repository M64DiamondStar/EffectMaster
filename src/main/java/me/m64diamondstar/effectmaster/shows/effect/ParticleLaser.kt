...
import me.m64diamondstar.effectmaster.locations.calculateCatmullRomPoint
...
class ParticleLaser: Effect() {
        ...
        try {
            ...
            EffectMaster.getFoliaLib().scheduler.runTimer({ task ->
                ...
                */
                if (duration / distance < frequency) {
                    ...
                    for (i2 in 1..entitiesPerTick.toInt())
                        if(smooth)
                            spawnParticle(calculateCatmullRomPoint(path, c + 1.0 / duration / entitiesPerTick * i2),
                                particle, amount, dX, dY, dZ, extra, force, players, effectShow, id)
                        else
                            spawnParticle(calculatePolygonalChain(path, c + 1.0 / duration / entitiesPerTick * i2),
                                particle, amount, dX, dY, dZ, extra, force, players, effectShow, id)
                }
                ...
                */
                    ...
                    if(smooth)
                        spawnParticle(calculateCatmullRomPoint(path, c), particle, amount, dX, dY, dZ, extra, force, players, effectShow, id)
                    else
                        spawnParticle(calculatePolygonalChain(path, c), particle, amount, dX, dY, dZ, extra, force, players, effectShow, id)
                }
    ...
    override fun getDescription(): String {
        return "Spawns a laser made out of particles."
    }
    ...
    override fun getDefaults(): List<Parameter> {
        ...
        list.add(Parameter(
            "BaseLocation",
            "world, 0, 0, 0",
            "The path the base of the laser follows using the format of " +
        ...
        list.add(Parameter(
            ...
            "The path which the particles follow using the format of " +
                    "\"world, x1, y1, z1; x2, y2, z2; x3, y3, z3\". You can of course repeat this process as much as you would like. Use a ; to separate different locations.",
            {it},
        ...
        list.add(Parameter(
            "ColorSequencer",
            "0: 0.0, 0.0, 0.0; 25: 0.0, 0.75, 0.0; 50: 0.3, 0.75, 0.0; 75: 0.0, 0.75, 0.0; 100: -0.3, 0.75, 0.0; 125: 0.0, 0.75, 0.0",
            "With this sequencer, you can edit the colors of the particle laser over time. " +
                    "The first number is the time in ticks, the second, third and fourth are the red, green and blue values. (ticks: red, green, blue)" +
                    "You can also add multiple values separated by semicolons. (ticks1:red1,green1,blue1; ticks2:red2,green2,blue2; ...)",
            {it},
            { Colors.getJavaColorSequencer(it) != null && Colors.getJavaColorSequencer(it)!!.isNotEmpty() })
        )
        list.add(Parameter(
            "Density",
            10,
            "How many particles each meter/block should contain. The higher the value, the more particles the laser will contain.",
            {it.toInt()},
            { it.toIntOrNull() != null && it.toInt() >= 1 })
        )
        list.add(Parameter(
            "Length",
            30,
            "The length of the laser starting from the base location, measured in blocks.",
            {it.toFloat()},
