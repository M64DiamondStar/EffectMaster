...
class UnlockSubCommand: SubCommand {
    ...
    override fun execute(sender: CommandSender, args: Array<String>) {
        ...
        if (!effectShow.locked) {
            sender.sendMessage(emComponent("<prefix><error>This show is not locked."))
            return
        }

        effectShow.locked = false

        sender.sendMessage(emComponent("<prefix><success>Successfully unlocked the show ${args[2]} in category ${args[1]}."))
    }
